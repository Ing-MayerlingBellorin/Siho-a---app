...
if pagina == "Registro Diario":
    ...
    if btn_save:
        if resp and desc:
            try:
                # El ID de tu carpeta Drive ya está referenciado aquí internamente
                ID_DRIVE = "1FD14E0qxiTv_zXn9rvSyJ0VsNJs8trYA"
                ...
                # EFECTO PROFESIONAL DE CARGA
                with st.spinner('Archivando en servidor SIHO-A...'):
                    time.sleep(1.5)
                
                # COLORES POR ACTIVIDAD
                if act == "Incidente":
                    st.warning(f"⚠️ REPORTE DE INCIDENTE ARCHIVADO. Requiere atención inmediata en {ubi}.")
                elif act == "Inspección":
                ...
                st.error(f"❌ ERROR DE CONEXIÓN: Asegúrate de que el Excel esté como 'Editor' para todos.")
        else:
